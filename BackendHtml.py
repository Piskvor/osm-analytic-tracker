    ...
    def print_state(self, db):
        ...
        if self.generation != db.generation:
            ...
            for c in db.chgsets_find(state=[db.STATE_CLOSED, db.STATE_OPEN, db.STATE_ANALYZING2,
                ...
                if 'mm' in c['labels']: #FIXME: does not belong here - this is configuration
                    csets_w_mm += 1
                if 'fix' in c['labels']: #FIXME: does not belong here - this is configuration
                    csets_w_fixes += 1
            ctx['csets_num_total'] = csets_total
            ctx['csets_with_notes'] = csets_w_notes
            ctx['csets_with_addr_changes'] = csets_w_addr_changes
            ctx['csets_with_mm'] = csets_w_mm
            ctx['csets_with_fixes'] = csets_w_fixes
